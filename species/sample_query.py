...
def doquery(query, index):
    ...
    with open(query) as q:
        for line in q:
            if len(line) > 100:
                (species, chromosome, source_file_start_offset, source_file_end_offset, segment_start_offset, segment_end_offset, orientation, data) = line.rstrip().split(' ', maxsplit=7)
