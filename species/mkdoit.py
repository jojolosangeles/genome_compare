...
def gen_script(files, segsize, wordlen, targetfolder, targetindex, samplesizepercent, numbersamples):
    ...
    print(f"{mkLogstash} > logstash_{targetindex}.confx")
    mkLogstash = f"sed -e \"s/__ELASTICSEARCH_INDEX__/{targetindex}/\" logstash_{targetindex}.confx"
    print(f"{mkLogstash} > logstash_{targetindex}.conf")
    # TODO: this is specific to my installation!  need this for repeated tests
    logstash_start_script = f"start_logstash_{targetindex}"
    print(
        f"echo 'rm /usr/local/Cellar/logstash/7.4.2/libexec/data/plugins/inputs/file/.sincedb*' > {logstash_start_script}")
    print(f"echo 'logstash -f logstash_{targetindex}.conf &' > {logstash_start_script}")
    ...
    for line in lines:
        ...
        print("echo 'sp,chr,loc,score,msp,mchr,mloc,orientation' > {sampleFilePath}.csv")
